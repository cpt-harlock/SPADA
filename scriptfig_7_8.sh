...
cat ./logs/sparsity_CAIDA1_m6_d1_HLL.txt | awk '{ sum += $3 } END { if (NR > 0) print "CAIDA1 m6 HLL sparsity: " sum / NR }' >> ./logs/avg_sparsities.txt;
cat ./logs/sparsity_CAIDA1_m7_d1_HLL.txt | awk '{ sum += $3 } END { if (NR > 0) print "CAIDA1 m7 HLL sparsity: " sum / NR }' >> ./logs/avg_sparsities.txt;

cat ./logs/sparsity_CAIDA2_m6_d1_HLL.txt | awk '{ sum += $3 } END { if (NR > 0) print "CAIDA2 m6 HLL sparsity: " sum / NR }' >> ./logs/avg_sparsities.txt;
cat ./logs/sparsity_CAIDA2_m7_d1_HLL.txt | awk '{ sum += $3 } END { if (NR > 0) print "CAIDA2 m7 HLL sparsity: " sum / NR }' >> ./logs/avg_sparsities.txt;
...
cat ./logs/sparsity_CAIDA1_m5_d1_DDSketch.txt | awk '{ sum += $3 } END { if (NR > 0) print "CAIDA1 m5 DDSketch sparsity: " sum / NR }' >> ./logs/avg_sparsities.txt;
cat ./logs/sparsity_CAIDA1_m6_d1_DDSketch.txt | awk '{ sum += $3 } END { if (NR > 0) print "CAIDA1 m6 DDSketch sparsity: " sum / NR }' >> ./logs/avg_sparsities.txt;

cat ./logs/sparsity_CAIDA2_m5_d1_DDSketch.txt | awk '{ sum += $3 } END { if (NR > 0) print "CAIDA2 m5 DDSketch sparsity: " sum / NR }' >> ./logs/avg_sparsities.txt;
cat ./logs/sparsity_CAIDA2_m6_d1_DDSketch.txt | awk '{ sum += $3 } END { if (NR > 0) print "CAIDA2 m6 DDSketch sparsity: " sum / NR }' >> ./logs/avg_sparsities.txt;
